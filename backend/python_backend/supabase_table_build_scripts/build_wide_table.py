...
def get_latest_complete_season():
    today = pd.Timestamp.now()
    candidate_years = [today.year, today.year - 1]
    for year in sorted(candidate_years, reverse=True):
        try:
            sched = nfl.load_schedules(seasons=[year]).to_pandas()
        except Exception:
            continue
        sb_games = sched[sched["game_type"] == "SB"]
        if not sb_games.empty and sb_games["home_score"].notna().all():
            return year
    return today.year - 1

START_SEASON = 2015
END_SEASON = get_latest_complete_season()
SEASON_TYPE = "REG"
...
def save_csv(df_wide: pd.DataFrame) -> str:
    ...
    print(f"Saved CSV -> {out_path}  ({df_wide.shape[0]} rows x {df_wide.shape[1]} cols)")
    return out_path
